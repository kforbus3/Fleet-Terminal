...
func (h *handler) setRolePermissions(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.d.Store.SetRolePermissions(r.Context(), id, rq.Permissions); err != nil {
		writeError(w, http.StatusInternalServerError, "could not set permissions")
		return
	}
...
func (h *handler) listPermissions(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		writeError(w, http.StatusInternalServerError, "could not list permissions")
		return
	}
	if perms == nil {
		perms = []models.Permission{}
	}
	writeJSON(w, http.StatusOK, map[string]any{"permissions": perms, "count": len(perms)})
}
